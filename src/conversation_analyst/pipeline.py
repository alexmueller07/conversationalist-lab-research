...
from conversation_analyst.media import audio as audio_io
from conversation_analyst.media.probe import probe
from conversation_analyst.media.sync import SyncResult, align_views
from conversation_analyst.session import CLOSE_VIEW, PERSONS, Session
from conversation_analyst.speech.asr import Transcript, transcribe
from conversation_analyst.speech.attribution import attribute_speakers
from conversation_analyst.speech.laughter import detect_laughter
from conversation_analyst.speech.prosody import analyze_prosody
from conversation_analyst.speech.vad import SileroVAD, probability_to_grid
from conversation_analyst.timeline import Segments
from conversation_analyst.turns import build_turn_set
...
def analyze_session(
    ...
    if "quality" not in skip:
        with stage_ctx("recording_quality") as stage:
            from conversation_analyst.media.quality import measure_audio_quality, measure_video_quality
    ...
    with stage_ctx("attribution") as stage:
        ...
        for person, track in face_tracks.items():
            from conversation_analyst.timeline import resample_to_grid
    ...
    if "semantics" not in skip and context.transcript is not None:
        with stage_ctx("semantics") as stage:
            from conversation_analyst.semantics import analyze_semantics
    ...
    if face_tracks:
        with stage_ctx("face_signals") as stage:
            from conversation_analyst.vision.nods import assign_roles
            from conversation_analyst.vision.signals import derive_face_signals
    ...
    if "body" not in skip:
        with stage_ctx("body_tracking") as stage:
            from conversation_analyst.vision.signals import derive_body_signals
...
def _arrays_to_face(data: dict[str, np.ndarray], role: str):
    from conversation_analyst.vision.tracker import FaceTrack
...
def _start_tracking(
    ...
    Returns a :class:`~conversation_analyst.isolate.TrackingPool`, or None when there is
    ...
    from conversation_analyst.isolate import TrackingPool, plan_workers
...
def _track_face_lazily(path: Path, model_dir: str, cfg: Config, role: str):
    ...
    """
    from conversation_analyst.vision.tracker import track_face
...
def _track_body_lazily(path: Path, model_dir: str, cfg: Config, role: str):
    from conversation_analyst.vision.tracker import track_body
...
def _arrays_to_body(data: dict[str, np.ndarray], role: str):
    from conversation_analyst.vision.tracker import BodyTrack
